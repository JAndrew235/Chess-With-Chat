...
public class Chessboard : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(ray, out info, 100, LayerMask.GetMask("Tile", "Hover", "Highlight")))
        {
            ...
            // if were hovering a tile after not hoving any tiles
            ...
            if (Input.GetMouseButtonDown(0))
            {
                if (chessPieces[hitPosition.x, hitPosition.y] != null)
                {
                    ...
                    //is iterator our Turn
                    if ((chessPieces[hitPosition.x, hitPosition.y].team == 0 && isWhiteTurn) || (chessPieces[hitPosition.x, hitPosition.y].team == 1 && !isWhiteTurn))
                    {
                        currentlyDragging = chessPieces[hitPosition.x, hitPosition.y];

                        // Get a list of where i can go, hightight tiles as well
                        availableMoves = currentlyDragging.GetAvailableMoves(ref chessPieces, TILE_COUNT_X, TILE_COUNT_Y);
                        //Get a list of spcail moves as well
            ...
            // If we are rleasing the mouse Button

            if (currentlyDragging != null && Input.GetMouseButtonUp(0))
            {
                Vector2Int previousPosition = new Vector2Int(currentlyDragging.currentX, currentlyDragging.currentY);

                bool validMove = MoveTo(currentlyDragging,
                    hitPosition.x,
                    hitPosition.y);
    ...
    private void PreventCheck()
    {
        ...
        SimulateMoveForSinglePiece(cp: currentlyDragging,
                                   ref availableMoves,
                                   targetKing);
    }
